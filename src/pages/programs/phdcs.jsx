...
import { fetchAllPHDCS } from "../../redux/actions/phdcs.action";
...
const PHDCS = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await dispatch(fetchAllPHDCS());
                if (response.type === 'phdcs/fetchAll/fulfilled') {
    ...
    const handleRoadmapClick = (termYear) => {
        navigate(`/roadmap/phdcs/${termYear}`);
    };
